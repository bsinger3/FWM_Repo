...
OUTPUT_DIR = legacy_raw_run_dir("superfithero_com")
...
SHOP_DOMAIN = "superfit-hero.myshopify.com"
...
def product_urls_from_sitemap() -> Tuple[List[str], List[Dict[str, object]]]:
    ...
    sitemap_urls = [
        html.unescape(match)
        for match in re.findall(r"<loc>(https://superfithero\.com/[^<]*sitemap_products_[^<]+)</loc>", sitemap_index)
    ]
    for sitemap_url in sitemap_urls:
        text = fetch_text(sitemap_url)
        page_urls = sorted(set(re.findall(r"https://superfithero\.com/products/[^<\s\"']+", text)))
...
def extract_superfit_size(custom_fields: Dict[str, str], text: str, variant: str) -> str:
    for key, value in custom_fields.items():
        if "super-fit" in key.lower() or "super fit" in key.lower():
...
def parse_review_rows(card: str, product: Dict[str, object], by_url: Dict[str, Dict[str, object]], fetched_at: str) -> List[Dict[str, str]]:
    ...
    size_display = extract_superfit_size(custom_fields, text_pool, variant)
