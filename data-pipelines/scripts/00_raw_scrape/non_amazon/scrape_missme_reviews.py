...
OUTPUT_DIR = legacy_raw_run_dir("missme_com")
...
def cached_products_from_summary(limit_products: Optional[int] = None) -> List[Dict[str, object]]:
    ...
    for item in summary.get("product_summaries") or []:
        if not isinstance(item, dict):
            continue
        product_id = norm(item.get("product_id") or item.get("shopify_product_id"))
        handle = norm(item.get("handle"))
        ...
        products.append(
            {
                "id": product_id,
                "handle": handle,
                "title": norm(item.get("title") or item.get("product_title")),
