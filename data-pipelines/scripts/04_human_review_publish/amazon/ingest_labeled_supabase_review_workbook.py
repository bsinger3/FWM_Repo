...
def require_openpyxl():
    ...
    try:
        ...
        raise SystemExit("openpyxl is required to ingest review workbooks. Install project dependencies and rerun.") from exc
...
def read_rows(workbook_path: Path) -> tuple[list[str], list[dict[str, object]]]:
    workbook = require_openpyxl()(workbook_path, data_only=False, read_only=True)
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("../FWM_Data/04_human_reviewed_ready_to_publish/legacy_approved_batches"),
    )
