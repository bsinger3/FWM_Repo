...
SITE_ROOT = "https://cozyearth.com"
DOMAIN = "cozyearth.com"
RETAILER = "cozyearth_com"
CATALOG_URL = f"{SITE_ROOT}/products.json"
OKENDO_STORE_ID = "db706fba-ba68-4327-8427-31a0cc53e58b"
...
APPAREL_RE = re.compile(
    r"\b(bra|dress|legging|shorts?|skirt|tank|tee|t-shirt|top|jacket|hoodie|sweatshirt|pant|jogger|pajama|robe|lounge|pullover)\b",
    re.I,
)
...
def product_context(product: Dict[str, object]) -> ProductContext:
    ...
    return ProductContext(
        ...
        brand=normalize_whitespace(product.get("vendor")) or "Cozy Earth",
...
def product_reviews_url(product_id: str) -> str:
    return (
        f"{OKENDO_API_ROOT}/stores/{OKENDO_STORE_ID}/products/shopify-{product_id}/reviews"
        "?limit=100&orderBy=has_media%20desc"
    )


def review_attribute_map(review: Dict[str, object]) -> Dict[str, str]:
    attrs: Dict[str, str] = {}
    product_attrs = review.get("productAttributes") if isinstance(review.get("productAttributes"), list) else []
    for item in product_attrs:
        ...
        if title and value:
            attrs[title.lower()] = value
    return attrs


def review_attribute_text(review: Dict[str, object]) -> str:
    return " ".join(f"{title}: {value}" for title, value in review_attribute_map(review).items())


def purchased_size(review: Dict[str, object]) -> str:
    attrs = review_attribute_map(review)
    return attrs.get("size purchased") or attrs.get("usual size") or ""
...
def row_for_review(context: ProductContext, review: Dict[str, object], media: Dict[str, str], fetched_at: str) -> Dict[str, str]:
    ...
    review_image = ReviewImage(
        ...
        review_id=f"cozyearth-okendo-{review_id}-{hashlib.md5(media['image_url'].encode('utf-8')).hexdigest()[:8]}",
        review_title=normalize_whitespace(review.get("title")),
        review_body=normalize_whitespace(" ".join([normalize_whitespace(review.get("body")), review_attribute_text(review)])),
        ...
        size_raw=purchased_size(review),
...
def collect_product_rows(
    ...
    pages_scanned = 0
    current_url = product_reviews_url(context.product_id)
    seen_review_ids = set()
    seen_media_urls = set()
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Scrape Cozy Earth public Okendo review images.")
