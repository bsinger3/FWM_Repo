...
OUTPUT_DIR = DATA_ROOT / "non-amazon" / "data" / "step_1_raw_scraping_data" / "holykitygirl_com"
OUTPUT_CSV = OUTPUT_DIR / "holykitygirl_com_reviews_matching_intake_schema.csv"
SUMMARY_JSON = OUTPUT_DIR / "holykitygirl_com_reviews_matching_intake_schema_summary.json"

SITE_ROOT = "https://holykitygirl.com"
SOURCE_SITE = f"{SITE_ROOT}/"
SHOP_DOMAIN = "holykitygirl.myshopify.com"
...
def product_urls_from_sitemap() -> Tuple[List[str], List[Dict[str, object]]]:
    ...
    sitemap_urls = [
        html.unescape(match)
        for match in re.findall(r"<loc>(https://holykitygirl\.com/[^<]*sitemap_products_[^<]+)</loc>", sitemap_index, re.I)
    ]
    sitemap_urls = [url for url in sitemap_urls if "/fr/" not in url and "/de/" not in url]
    for sitemap_url in sitemap_urls:
        try:
            ...
            sources.append({"source": "product_sitemap", "url": sitemap_url, "count": 0, "error": str(exc)})
            continue
        page_urls = sorted(set(re.findall(r"https://holykitygirl\.com/products/[^<\s\"']+", text, re.I)))
...
def extract_holykitygirl_size(custom_fields: Dict[str, str], text: str, variant: str) -> str:
...
def output_skip_reason(product: Dict[str, object]) -> str:
    ...
    if product_type in {"accs", "bag", "bags", "necklace", "earrings", "shoes", "shoe", "boots"}:
        return "out_of_scope_accessory"
    if any(term in title for term in ["necklace", "earrings", "handbag", "waist chain", "body chain"]):
        return "out_of_scope_accessory"
    return ""
...
def parse_review_rows(card: str, product: Dict[str, object], by_url: Dict[str, Dict[str, object]], fetched_at: str) -> List[Dict[str, str]]:
    ...
    size_display = extract_holykitygirl_size(custom_fields, text_pool, variant)
    ...
    for index, product in enumerate(products, start=1):
        ...
        if product_id:
            while True:
                ...
                page_numbers = [
                    int(value)
                    for value in re.findall(r"data-page=(['\"])(\d+)\1", html.unescape(html_text), re.I)
