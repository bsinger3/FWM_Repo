...
SITE_ROOT = "https://www.wedtrend.com"
RETAILER = "wedtrend_com"
SHOP_DOMAIN = "wedtrend.myshopify.com"
...
APPAREL_RE = re.compile(
    r"\b(bridesmaid|cocktail|dress|dresses|formal|gown|jumpsuit|maxi|midi|mini|prom|romper|wedding)\b",
    re.I,
)
...
def discover_product_urls() -> Tuple[List[str], Dict[str, object]]:
    ...
    for page in range(1, 10000):
        try:
            payload = request_json(f"{SITE_ROOT}/products.json?limit=250&page={page}")
        except Exception:
            break
        batch = payload.get("products") if isinstance(payload.get("products"), list) else []
        source_pages.append({"url": f"{SITE_ROOT}/products.json?limit=250&page={page}", "products": len(batch)})
        if not batch:
            break
        for product in batch:
            ...
            if not handle:
                continue
            product_url = canonical_product_url(f"{SITE_ROOT}/products/{handle}")
            if product_url not in seen:
                seen.add(product_url)
                products.append(product_url)
        print(f"[catalog page {page}] products={len(batch)} total={len(products)}", flush=True)
        if len(batch) < 250:
            break
    ...
    sitemap_source_pages: List[Dict[str, object]] = []
    for sitemap_url in product_sitemaps:
        normalized_sitemap_url = sitemap_url.replace("&amp;", "&")
        try:
            xml = request_text(normalized_sitemap_url)
        except Exception as exc:
            sitemap_source_pages.append({"url": normalized_sitemap_url, "products": 0, "error": str(exc)})
            continue
        ...
        sitemap_source_pages.append({"url": normalized_sitemap_url, "products": len(product_urls)})
        for product_url in product_urls:
            if product_url and product_url not in seen:
                seen.add(product_url)
                products.append(product_url)
    return products, {
        "shopify_products_json": source_pages,
        "sitemap_index": f"{SITE_ROOT}/sitemap.xml",
        "product_sitemaps": sitemap_source_pages,
...
def parse_review(block: str, fetched_at: str) -> List[Dict[str, str]]:
    ...
    for index, image_url in enumerate(images, start=1):
        ...
        review = ReviewImage(
            image_url=image_url,
            review_id=f"wedtrend-judgeme-{review_id}-{index}" if review_id else f"wedtrend-judgeme-{fallback_id}-{index}",
            review_title=title,
            review_body=body,
...
def scrape(args: argparse.Namespace) -> Tuple[List[Dict[str, str]], Dict[str, object]]:
    ...
    pages_scanned = 0
    zero_image_page_streak = 0
    stopped_after_zero_image_streak = False
    ...
    for page in range(1, args.max_review_pages + 1):
        ...
        image_mentions = html.count("judgeme.imgix.net")
        review_pages.append({"page": page, "blocks": len(blocks), "html_bytes": len(html), "image_mentions": image_mentions})
        if not blocks:
            break
        if image_mentions:
            zero_image_page_streak = 0
        else:
            zero_image_page_streak += 1
            if args.stop_after_zero_image_pages and zero_image_page_streak >= args.stop_after_zero_image_pages:
                stopped_after_zero_image_streak = True
                print(
                    f"[review page {page}] zero image page streak reached {zero_image_page_streak}; stopping media scan",
                    flush=True,
                )
                break
        pages_scanned += 1
    ...
    finished_at = utc_now()
    exhaustive = not errors and not args.limit_review_pages and bool(review_pages) and (
        review_pages[-1].get("blocks") == 0 or stopped_after_zero_image_streak
    )
    return rows, {
        "site": SITE_ROOT,
        "retailer": RETAILER,
        ...
        "media_feed_last_image_page": max((item["page"] for item in review_pages if item.get("image_mentions")), default=0),
        "stopped_after_zero_image_page_streak": stopped_after_zero_image_streak,
        "zero_image_page_streak_limit": args.stop_after_zero_image_pages,
        ...
        "scrape_scope_status": "public_judgeme_media_feed_exhausted_by_zero_image_streak"
        if stopped_after_zero_image_streak
        else ("full_public_judgeme_media_feed_complete" if exhaustive else "stopped_or_limited"),
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Scrape Wedtrend customer review media from public Judge.me feed.")
    parser.add_argument("--max-review-pages", type=int, default=500)
    parser.add_argument("--limit-review-pages", type=int, default=0)
    parser.add_argument("--stop-after-zero-image-pages", type=int, default=30)
