...
OUTPUT_DIR = DATA_ROOT / "non-amazon" / "data" / "step_1_raw_scraping_data" / "universalstandard_com"
OUTPUT_CSV = OUTPUT_DIR / "universalstandard_com_reviews_matching_intake_schema.csv"
SUMMARY_JSON = OUTPUT_DIR / "universalstandard_com_reviews_matching_intake_schema_summary.json"

SITE_ROOT = "https://www.universalstandard.com"
SOURCE_SITE = f"{SITE_ROOT}/"
PRODUCTS_JSON_URL = f"{SITE_ROOT}/products.json"
SITEMAP_URL = f"{SITE_ROOT}/sitemap.xml"
YOTPO_APP_KEY = "jIFTSQDKKpIDorJipCqCUf95WrenWpxc3l0wPUc7"
...
def fetch_products(limit_products: Optional[int] = None) -> Tuple[List[Dict[str, object]], List[Dict[str, object]]]:
    ...
    sitemap_urls = [html.unescape(u) for u in re.findall(r"<loc>(https://www\.universalstandard\.com/[^<]*sitemap_products_[^<]+)</loc>", sitemap_index)]
    sitemap_product_urls: List[str] = []
    for sitemap_url in sitemap_urls:
        text = fetch_text(sitemap_url)
        urls = sorted(set(re.findall(r"https://www\.universalstandard\.com/products/[^<\s\"']+", text)))
...
def parse_review_measurements(review: Dict[str, object], body: str) -> Dict[str, str]:
    ...
    joined = " ".join(part for part in [body, fields.get("height", ""), fields.get("age", ""), fields.get("typical size with us", "")] if part)
...
def parse_size(review: Dict[str, object], body: str) -> str:
    fields = custom_fields(review)
    if fields.get("size purchased"):
        return fields["size purchased"].upper()
    if fields.get("size"):
...
def clothing_type(product: Dict[str, object]) -> str:
    ...
    if "short" in hay or "skort" in hay:
        return "shorts"
    if any(token in hay for token in ["top", "corset", "bodysuit", "tee", "shirt", "blouse", "tank", "vest", "turtleneck"]):
        return "top"
    if "pant" in hay or "trouser" in hay or "legging" in hay:
        return "pants"
    if "jumpsuit" in hay or "romper" in hay:
        return "jumpsuit"
    if "jacket" in hay or "coat" in hay or "blazer" in hay or "outerwear" in hay:
        return "outerwear-light"
    if "sleep" in hay or "lounge" in hay:
        return "loungewear"
    return ""
...
def skip_reason(product: Dict[str, object]) -> str:
    ...
    if product_type in {"accessories", "shoe", "shoes"} or re.search(r"\b(heels?|sandals?|boots?|bags?|purses?|earrings?|necklace|bracelet|hat|cap|sunglasses|gloves?|stickers?|gift box)\b", title):
...
def main() -> None:
    ...
    summary = {
        ...
        "rows_filtered_grouped_review_duplicates": grouped_review_duplicates,
        "dedupe_key": "review id + original_url_display",
