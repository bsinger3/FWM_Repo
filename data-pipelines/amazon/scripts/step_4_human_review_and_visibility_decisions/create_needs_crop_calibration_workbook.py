#!/usr/bin/env python3
"""Create a one-question NEEDS_CROP calibration workbook."""
...
OUTPUT_XLSX = EXP_DIR / "needs_crop_yes_no_review_queue.xlsx"
...
def needs_crop_truth(row: dict[str, str]) -> bool:
...
def build_candidates(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    candidates = [
        row
        for row in rows
        if needs_crop_truth(row) or model_predicted_needs_crop(row)
    ]
    return sorted(
        candidates,
        key=lambda row: (
            not model_predicted_needs_crop(row),
...
def value_for(row: dict[str, str], header: str) -> object:
    ...
    if header == "model_predicted_needs_crop":
        return model_predicted_needs_crop(row)
    if header == "model_rule":
        return "YES if seg_mask_area_pct < 0.20"
