...
function setRowDecisions(rows, patchOrFactory) {
...
function hasRejectReason() {
  return Boolean(el.rejectReason.value);
}

function requireRejectReason() {
  if (hasRejectReason()) return true;
...
function applyClickMode(row) {
  ...
  if (state.clickMode === "reject") {
    if (!requireRejectReason()) return true;
    const current = getRowDecision(row);
    setRowDecision(row, {
...
function updateSourceSummary() {
  const bucketCount = Object.values(state.parts.buckets)
    .map((bucket) => `${bucket.label}: ${bucket.parts.length} parts`)
    .join(" | ");
...
function renderTabs() {
  ...
  for (const bucket of ["approve_candidates", "needs_human_review", "disapprove_candidates"]) {
    ...
    button.type = "button";
    button.textContent = `${config.label} (${config.parts.length})`;
    button.addEventListener("click", async () => {
      state.bucket = bucket;
      state.part = config.parts[0]?.part || "001";
...
function renderPartSelect() {
  ...
  for (const part of parts) {
    ...
    option.textContent = `Part ${part.part}`;
...
async function loadRows() {
  ...
  fillReasonSelect(el.rejectReason);
  fillReasonSelect(el.detailRejectReason, true);
  renderReasonFilter();
...
function applyToSelected(patch) {
  setRowDecisions(selectedRows(), patch);
}
...
function applyToVisibleUnmarked(patch) {
  setRowDecisions(visibleUnmarkedRows(), patch);
}
...
function saveDetailPatch(patch) {
  ...
  setRowDecision(row, {
    ...patch,
    rejectionReason: patch.rejectionReason ?? el.detailRejectReason.value,
...
async function saveProgress() {
  ...
  for (const decision of decisions) {
    if (decision.humanState === "DISAPPROVE" && !decision.rejectionReason) {
      alert(`Rejected row is missing a rejection reason:\n${decision.rowKey}`);
      return;
    }
  }
...
function bindEvents() {
  ...
  el.rejectSelected.addEventListener("click", () => {
    if (!requireRejectReason()) return;
    applyToSelected({ humanState: "DISAPPROVE", rejectionReason: el.rejectReason.value });
  });
  ...
  el.rejectVisibleUnmarked.addEventListener("click", () => {
    if (!requireRejectReason()) return;
    applyToVisibleUnmarked({ humanState: "DISAPPROVE", rejectionReason: el.rejectReason.value });
  });
  ...
  for (const button of [el.clickModeInspect, el.clickModeApprove, el.clickModeReject]) {
    button.addEventListener("click", () => {
      if (button.dataset.clickMode === "reject" && !requireRejectReason()) return;
  ...
  window.addEventListener("keydown", (event) => {
    ...
    if (event.key.toLowerCase() === "d" && requireRejectReason()) applyToSelected({ humanState: "DISAPPROVE", rejectionReason: el.rejectReason.value });
  ...
  window.addEventListener("pointerup", () => {
    ...
    if (state.drag.moved) {
      ...
      if (card?.dataset.key) {
        ...
        else state.selected.add(card.dataset.key);
      }
    }
    state.drag = null;
