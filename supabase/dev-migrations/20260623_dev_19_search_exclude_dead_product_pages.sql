-- Dev-only: make match_by_measurements exclude images whose product page is dead.
--
-- Background: the search RPC previously hid "dead" images ONLY via an image_reports
-- row (reason='dead_link', anon_id='manual_product_category_review_2026_05_20'). That
-- forced automated dead-page detection (e.g. the Amazon /dp/ 404 backfill) to reuse a
-- manual-review anon_id just to make the hide take effect. This adds a clean, set-based
-- exclusion: any image whose product page has source_status in the "unavailable" set
-- (page_not_found / product_unavailable / redirected_to_non_product) is dropped from
-- search. Now marking staging.product_pages.source_status='page_not_found' is sufficient
-- to hide its images — no image_reports hack required.
--
-- Only the `prepared` CTE's WHERE gains one `not exists (... staging.product_pages ...)`
-- clause; everything else is byte-identical to 20260616_dev_03.
...
AS $function$
  with prepared as (
    ...
    where
      ...
      -- NEW: drop images whose product page is known-dead/unavailable.
      and not exists (
        select 1
