-- Dev-only: split search results into 'matches' vs 'close matches'.
--
-- The measurement inputs (height/weight/bust/waist/hips) only RANK the eligible
-- catalog, they don't filter it, so the old total_count = whole catalog and the
-- header was misleading. Now the function also returns, per row, matches_any
-- (true when within tolerance of >=1 entered measurement: height/weight ... ±2/±10)
-- and match_count (how many rows match >=1). Results are sorted matches-first, so
-- the UI shows 'Found <match_count> results', then a 'close matches' divider when
-- you scroll past the matches into the ranked remainder. total_count is kept.
--
-- Function-only rebuild on top of dev_28 (matview unchanged). Return shape grows
-- (matches_any, match_count) so the prior signatures are dropped first. Preserves
-- every earlier fix: low-res gate, broad anti-join, tiebreaker, sparse hard
-- filters, total_count. Apply LAST; numbered dev_29.
...
as $function$
  with scored as (
    select
      ...
      (
        (in_height is not null and s.height_in_display is not null and abs(s.height_in_display - in_height) <= 2)
        or (in_weight is not null and s.weight_lbs_display is not null and abs(s.weight_lbs_display - in_weight) <= 10)
        or (in_bust is not null and s.bust_in_number_display is not null and abs(s.bust_in_number_display - in_bust) <= 2)
        or (in_waist is not null and s.waist_in_numeric is not null and abs(s.waist_in_numeric - in_waist) <= 2)
        or (in_hips is not null and s.hips_in_display is not null and abs(s.hips_in_display - in_hips) <= 2)
      ) as matches_any
